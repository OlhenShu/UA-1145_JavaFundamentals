...
//public class Car {//for immutable class type can be record
...
//    }
//}
public record Car(String type, int yearOfProduction, int engineCapacity) {//like this
    ...
    static void findModelByYear(Car[] cars) {
        ...
        int year = scanner.nextInt();
        for (Car car : cars) {
            if (year == car.yearOfProduction()) {
                System.out.println("Model " + car.type() + " was made at " + year);
            }
        }
    }
    ...
    static void sortByProductionYearDesc(Car[] cars) {
        for (int i = 0; i < cars.length; i++) {
            for (int j = i + 1; j < cars.length; j++) {
                if (cars[j].yearOfProduction() > cars[i].yearOfProduction()) {
                    Car temp = cars[i];
                    cars[i] = cars[j];
                    cars[j] = temp;
                }
        ...
        System.out.println("Cars sorted by year of production: ");
        for (Car car : cars) {
            System.out.println("Model " + car.type() + " year " + car.yearOfProduction() +
